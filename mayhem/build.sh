...
# DEBUG_FLAGS: explicit DWARF-3 (clang-19 defaults to DWARF-5; Mayhem triage
# needs DWARF < 4). Must come AFTER $SANITIZER_FLAGS in compile/link lines so
# our -gdwarf-3 wins over any -g baked into SANITIZER_FLAGS.
...
EXTRA="$SANITIZER_FLAGS $DEBUG_FLAGS -fsanitize=fuzzer-no-link -DPRINTF_ENABLED -Wno-error=undefined-internal -Wno-error"
...
build_harness() {
    ...
    $CC $SANITIZER_FLAGS $DEBUG_FLAGS $defs -DPRINTF_ENABLED -DARCH_FLASH_OFFSET=0 $INCS \
...
$CC $SANITIZER_FLAGS $DEBUG_FLAGS $INCS -DARCH_FLASH_OFFSET=0 \
